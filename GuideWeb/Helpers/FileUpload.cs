...
namespace GuideWeb.Helpers
{
    public class FileUpload : IFileUpload
    {
        ...
        private void ImageSave(IFormFile file, ref string UrlPath, ref string FileName)
        {
            ...
            {
                var filePath = Path.Combine(Directory.GetCurrentDirectory(), $"wwwroot\\Panel\\img\\AddImagePanel");
                if (!Directory.Exists(filePath))
                {
                    Directory.CreateDirectory(filePath);
                }
                FileName = ReplacedFileName(file.FileName);
                var path = Path.Combine(_webHost.WebRootPath, $"Panel\\img\\AddImagePanel\\", $"{subGuid}-{FileName}");
